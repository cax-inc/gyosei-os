...
export async function generateSiteContent(input: GenerateInput): Promise<SiteContent> {
  ...
  const headlineGuide = `メインキャッチコピー（完結した1文）

以下の8つのパターンから最も適切なものを選び、{事務所名}に実際の情報を当てはめて生成してください。
型はそのままコピーせず、自然な日本語になるよう微調整してよい。

「手続きの不安を、安心に変える行政書士」
「複雑な申請も、あなたの代わりにわかりやすく」
「複雑な申請こそ、経験豊富な行政書士へ」
「{事務所名}は、あなたの『わからない』を一つずつ解決します」
「手続きの壁を一緒に乗り越える身近なパートナー、{事務所名}」
「申請の不安をなくし、未来への一歩を支えます」
「書類の悩みも申請の不安も、{事務所名}がまとめて解決」
「人生の転機にお手伝いがしたい、その思いから{事務所名}を開業しました」
