...
export async function generateSiteContent(input: GenerateInput): Promise<SiteContent> {
  ...
  const headlineGuideMap: Record<string, string> = {
    '信頼感重視': `メインキャッチコピー（2行・各行15字以内）
【型】「〇〇の手続き、\n誠実にお任せください」「〇〇でお困りなら、\n確かな経験でお応えします」「大切な〇〇を、\n丁寧にサポートします」
【ルール】\\nで2行に分割すること。文末は「〜ください」「〜します」「〜いたします」などの述語で終わること。助詞（〜を・〜に・〜が）で終わる体言止めは禁止。感嘆符禁止。エリア名または業務名を必ず含める。`,

    '親しみやすい': `メインキャッチコピー（2行・各行12字以内・ひらがな多め）
【型】「〇〇のこと、\n一人で悩まないで」「まず話して\nみませんか？」「あなたの街の\n身近な相談相手」
【ルール】\\nで2行に分割すること。問いかけ形または呼びかけ形。難しい漢字を避ける。温かみのある言葉を使う。`,

    '専門性重視': `メインキャッチコピー（2行・各行15字以内）
【型】「〇〇申請専門。\n要件から書類まで完全対応」「〇〇に特化した\n行政書士事務所」「他所で断られた〇〇案件、\nご相談ください」
【ルール】\\nで2行に分割すること。業務名を明示。「専門」「特化」「完全対応」などの専門性を示す言葉を使う。感情表現禁止。`,

    '実績重視': `メインキャッチコピー（2行・各行13字以内・数字を含める）
【型】「〇〇件の実績が\n証明する確かな申請力」「許可取得実績〇年。\n結果でお応えします」「年間〇〇件対応。\n〇〇なら当事務所へ」
【ルール】\\nで2行に分割すること。具体的な数字（件数・年数）を必ず含める。断言形。「実績」「証明」「結果」などの言葉を使う。`,
  }
